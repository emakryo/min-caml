...
let rec g env (r, e) = (* β簡約ルーチン本体 (caml2html: beta_g) *)
  let e' = match e with
    | Unit -> Unit
    | Int(i) -> Int(i)
    | Float(d) -> Float(d)
    ...
    | Lsl(x, y) -> Lsl(find x env, find y env)
    | Lsr(x, y) -> Lsr(find x env, find y env)
    | Lor(x, y) -> Lor(find x env, find y env)
    | Land(x, y) -> Land(find x env, find y env)
