...
let rec g env (r, e) = (* 式の仮想マシンコード生成 (caml2html: virtual_g) *)
  let e' = match e with
    ...
    | Closure.Float(d) ->
	 ...
	 with Not_found ->
	   ...
	   l in
       let x = Id.genid "l" in
       Let((x, Type.Int), SetL(l), (r, Ans(LdDF(x, C(0), 1))))
    | Closure.Neg(x) -> Ans(Neg(x))
    | Closure.Add(x, y) -> Ans(Add(x, V(y)))
    | Closure.Sub(x, y) -> Ans(Sub(x, V(y)))
    (* | Closure.Mul(x, y) -> Ans(Mul(x, V(y))) *)
    (* | Closure.Div(x, y) -> Ans(Div(x, V(y))) *)
    | Closure.Lsl(x, y) -> Ans(Lsl(x, V(y)))
    | Closure.Lsr(x, y) -> Ans(Lsr(x, V(y)))
    ...
    | Closure.MakeCls((x, t), { Closure.entry = l; Closure.actual_fv = ys }, e2) -> (* クロージャの生成 (caml2html: virtual_makecls) *)
	 ...
	 expand
       ...
       Let((x, t), Mov(reg_hp),
	   (r, Let((reg_hp, Type.Int), Add(reg_hp, C(align offset)),
		   let z = Id.genid "l" in
		   (r, Let((z, Type.Int), SetL(l),
		       seq(St(z, x, C(0), 1),
			   store_fv))))))
    | Closure.AppCls(x, ys) ->
    ...
    | Closure.LetTuple(xts, y, e2) ->
	 ...
	 expand
	   ...
	   (fun x t offset load ->
	    if not (S.mem x s) then load else (* [XX] a little ad hoc optimization *)
	      (fst load, Let((x, t), Ld(y, C(offset), 1), load))) in
       snd load
...
let h { Closure.name = (Id.L(x), t); Closure.args = yts; Closure.formal_fv = zts; Closure.body = e } =
  ...
  let (offset, load) =
    expand
      ...
      (fun z t offset load -> (fst load, Let((z, t), Ld(reg_cl, C(offset), 1), load))) in
  match t with
  | Type.Fun(_, t2) ->
