...
and inst = (* 命令の列 (caml2html: sparcasm_t) *)
  | Ans of exp
...
let fletd(x, e1, e2) = (fst e2, Let((x, Type.Float), e1, e2))
let seq(e1, e2) = (fst e2, Let((Id.gentmp Type.Unit, Type.Unit), e1, e2))
...
let rec fv_exp = function
  ...
  | Add(x, y') | Sub(x, y') | Lsl(x, y') | Lsr(x, y')| Ld(x, y', _) | LdDF(x, y', _) -> x :: fv_id_or_imm y'
...
let rec concat (r1, e1) xt e2 =
  match e1 with
  | Ans(exp) -> (r1, Let(xt, exp, e2))
  | Let(yt, exp, e1') -> (r1, Let(yt, exp, concat e1' xt e2))
