...
let rec g env (r, e) = (* 命令列の即値最適化 (caml2html: simm13_g) *)
  let e' = match e with
...
and g' env = function (* 各命令の即値最適化 (caml2html: simm13_gprime) *)
  ...
  | Lsl(x, V(y)) when M.mem y env -> Lsl(x, C(M.find y env))
  | Lsl(x, V(y)) when M.mem x env -> Lsl(y, C(M.find x env))
  | Lsr(x, V(y)) when M.mem y env -> Lsr(x, C(M.find y env))
  | Lsr(x, V(y)) when M.mem x env -> Lsr(y, C(M.find x env))
