...
and g' dest cont regenv r exp = (* 各命令のレジスタ割り当て (caml2html: regalloc_gprime) *)
  ...
  in
  ((r, e'), regenv')
and g'_if dest cont regenv r exp constr e1 e2 = (* ifのレジスタ割り当て (caml2html: regalloc_if) *)
  ...
  in
  (snd e'', regenv'')
and g'_call dest cont regenv r exp constr ys zs = (* 関数呼び出しのレジスタ割り当て (caml2html: regalloc_call) *)
  let (e', regenv') = 
    (List.fold_left
     (fun e x ->
