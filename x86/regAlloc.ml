...
let rec target' src (dest, t) = function
  ...
  | CallDir(_, ys, zs) ->
      true, (target_args src regs 0 ys @
	     target_args src fregs 0 zs)
  | _ -> false, []
and target src dest (r, e) = (* register targeting (caml2html: regalloc_target) *)
  match e with
...
let rec source t (r, e) = 
  match e with
  | Ans(exp) -> source' t exp
  | Let(_, _, e) -> source t e
and source' t = function
  | Mov(x) | Neg(x) | Add(x, C _) | Sub(x, _) | Lsl(x, _) | Lsr(x, _) | FMovD(x) | FNegD(x) | FSubD(x, _) | FDivD(x, _) -> [x]
...
let rec g dest cont regenv (r, e) = (* 命令列のレジスタ割り当て (caml2html: regalloc_g) *)
  match e with
  | Ans(exp) -> g'_and_restore dest cont regenv r exp
  | Let((x, t) as xt, exp, e) ->
     assert (not (M.mem x regenv));
     let cont' = concat e dest cont in
     let (e1', regenv1) = g'_and_restore xt cont' regenv r exp in
...
and g'_and_restore dest cont regenv r exp = (* 使用される変数をスタックからレジスタへRestore (caml2html: regalloc_unspill) *)
  try g' dest cont regenv r exp
  with NoReg(x, t) ->
    ((* Format.eprintf "restoring %s@." x; *)
      g dest cont regenv (r, Let((x, t), Restore(x), (r, Ans(exp)))))
and g' dest cont regenv r exp = (* 各命令のレジスタ割り当て (caml2html: regalloc_gprime) *)
  let (e', regenv') = match exp with
    ...
    | Lsl(x, y') -> (Ans(Lsl(find x Type.Int regenv, find' y' regenv)), regenv)
    | Lsr(x, y') -> (Ans(Lsr(find x Type.Int regenv, find' y' regenv)), regenv)
    ...
    | IfEq(x, y', e1, e2) as exp -> g'_if dest cont regenv r exp (fun e1' e2' -> IfEq(find x Type.Int regenv, find' y' regenv, e1', e2')) e1 e2
    | IfLE(x, y', e1, e2) as exp -> g'_if dest cont regenv r exp (fun e1' e2' -> IfLE(find x Type.Int regenv, find' y' regenv, e1', e2')) e1 e2
    | IfGE(x, y', e1, e2) as exp -> g'_if dest cont regenv r exp (fun e1' e2' -> IfGE(find x Type.Int regenv, find' y' regenv, e1', e2')) e1 e2
    | IfFEq(x, y, e1, e2) as exp -> g'_if dest cont regenv r exp (fun e1' e2' -> IfFEq(find x Type.Float regenv, find y Type.Float regenv, e1', e2')) e1 e2
    | IfFLE(x, y, e1, e2) as exp -> g'_if dest cont regenv r exp (fun e1' e2' -> IfFLE(find x Type.Float regenv, find y Type.Float regenv, e1', e2')) e1 e2
    | CallCls(x, ys, zs) as exp -> g'_call dest cont regenv r exp (fun ys zs -> CallCls(find x Type.Int regenv, ys, zs)) ys zs
    | CallDir(l, ys, zs) as exp -> g'_call dest cont regenv r exp (fun ys zs -> CallDir(l, ys, zs)) ys zs
    | Save(x, y) -> assert false
  in
  ((r, e'), regenv')
and g'_if dest cont regenv r exp constr e1 e2 = (* if¤Î¥ì¥¸¥¹¥¿³ä¤êÅö¤Æ (caml2html: regalloc_if) *)
  ...
  in
  (snd e'', regenv'')
and g'_call dest cont regenv r exp constr ys zs = (* ´Ø¿ô¸Æ¤Ó½Ð¤·¤Î¥ì¥¸¥¹¥¿³ä¤êÅö¤Æ (caml2html: regalloc_call) *)
  let (e'', regenv'') = (List.fold_left
     (fun e x ->
...
let h { name = Id.L(x); args = ys; fargs = zs; body = e; ret = t } = (* 関数のレジスタ割り当て (caml2html: regalloc_h) *)
  ...
  let a =
    ...
    | Type.Float -> fregs.(0)
    | _ -> regs.(0) in
  let (e', regenv') = g (a, t) (fst e, Ans(Mov(a))) regenv e in
  { name = Id.L(x); args = arg_regs; fargs = farg_regs; body = e'; ret = t }

let f (Prog(data, fundefs, e)) = (* プログラム全体のレジスタ割り当て (caml2html: regalloc_f) *)
  Format.eprintf "register allocation: may take some time (up to a few minutes, depending on the size of functions)@.";
  let fundefs' = List.map h fundefs in
  let e', regenv' = g (Id.gentmp Type.Unit, Type.Unit) (fst e, Ans(Nop)) M.empty e in
  Prog(data, fundefs', e')
